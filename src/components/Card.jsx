...
export const ProjectCard = ({ project }) => {
    return (
        <Card className="overflow-hidden group">
            {/* Image Container with Popup Links */}
            <div className="relative aspect-video mb-4 rounded-lg overflow-hidden bg-bg-abyss">
                <img
                    src={project.image}
                    alt={project.title}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-bg-abyss/90 via-bg-abyss/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                    ...
                    <div className="flex items-center justify-center gap-3">
                        {project.demo && (
                            <a
                                ...
                                className="flex items-center gap-2 px-4 py-2 rounded-full bg-accent-crimson text-white font-medium text-sm hover:bg-accent-crimson/80 transition-all duration-200 hover:scale-105 shadow-lg shadow-accent-crimson/25"
                            >
                        ...
                        {project.github && (
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm text-white font-medium text-sm hover:bg-white/20 transition-all duration-200 hover:scale-105 border border-white/20"
                            >
            ...
            {/* Title */}
            <h3 className="font-display text-xl font-bold text-text-white mb-2">
                {project.title}
            </h3>
            ...
            {/* Tags - Using site palette */}
            <div className="flex flex-wrap gap-2">
                {project.tags.slice(0, 4).map((tag, index) => (
                    <span
                        key={index}
                        className="px-3 py-1 text-xs rounded-full font-medium bg-accent-crimson/15 text-accent-crimson"
                    >
                        {tag}
                    </span>
                ))}
                {project.tags.length > 4 && (
                    <span className="px-3 py-1 text-xs rounded-full font-medium bg-accent-sapphire/15 text-accent-sapphire">
